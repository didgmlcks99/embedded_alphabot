...
int main(){
    ...
    while(true) {
        PWMR.pulsewidth_ms(500);
        PWML.pulsewidth_ms(500);
    }
}
//====================================================
//==================initialization====================
...
void Actuator(){
    ...
    if(weight >= (2000/den)*3/4){
        if(direction == LEFT){
        PWML.pulsewidth_us(L_PWM+3+0.0*weight);
        PWMR.pulsewidth_us(R_PWM-2.1*weight);
        }
        else{
        PWML.pulsewidth_us(L_PWM+3-2.1*weight);
        PWMR.pulsewidth_us(R_PWM+0.0*weight);
        }
    }else{
    
        if(direction == LEFT){
        PWML.pulsewidth_us(L_PWM+3+0.0*weight);
        PWMR.pulsewidth_us(R_PWM-1.5*weight);
        }
        else{
        PWML.pulsewidth_us(L_PWM+3-1.5*weight);
        PWMR.pulsewidth_us(R_PWM+0.0*weight);
        }
    }
    ...
    {  
        if(P_weight >=(2000/den)*2/3 && P_weight<=(2000/den)*4/5){
            if(P_direction == LEFT){
            PWML.pulsewidth_us(L_PWM-P_weight+3);
            PWMR.pulsewidth_us(R_PWM); 
            }
            else{
            PWML.pulsewidth_us(L_PWM+3);
            PWMR.pulsewidth_us(R_PWM-P_weight);
            }
        }
    if(P_weight>=(2000/den)*4/5){
            if(P_direction == LEFT){
            PWML.pulsewidth_us(L_PWM+3-1.0*P_weight);
            PWMR.pulsewidth_us(R_PWM+1.0*P_weight); 
            }
            else{
            PWML.pulsewidth_us(L_PWM+3+1.0*P_weight);
            PWMR.pulsewidth_us(R_PWM-1.0*P_weight);
...
void Distance_check(){
   ...
   //pc.printf("distance = %d\r\n",flag_distance);
    ...
    if(flag_distance == YOK){
     while(1){ 
            PWMR.pulsewidth_us(100);
            PWML.pulsewidth_us(100);
            LIN2.write(1);
            LIN1.write(0);
            RIN3.write(0);
            RIN4.write(1);
...
if(Otime.read() <= 0.5){
         ...
         if(dir == 0){
             PWML.pulsewidth_us(L_PWM-20);
             PWMR.pulsewidth_us(R_PWM-100); 
              }
         else if(dir == 1 ){
             PWML.pulsewidth_us(L_PWM-100);
             PWMR.pulsewidth_us(R_PWM-30);
           }
           
}else if(Otime.read() <= 1.0){

         if(dir == 0){
             PWML.pulsewidth_us(L_PWM+5);
             PWMR.pulsewidth_us(R_PWM); 
              }
         else if(dir == 1 ){
             PWML.pulsewidth_us(L_PWM+5);
